...
socket.on('game-state', (data) => {
  document.getElementById('estado').textContent =
    'Jugadores conectados: ' + (data.jugadores?.length || 0) + (data.partidaIniciada ? ' | Partida iniciada' : '');
});

// Placeholder para pregunta
socket.on('question', (data) => {
  document.getElementById('pregunta').textContent = data.pregunta || '(Sin pregunta)';
  document.getElementById('opciones').textContent = '(Opciones...)';
});

// Placeholder para game-over
socket.on('game-over', (data) => {
  document.getElementById('estado').textContent = '¡Ganador: ' + (data.ganador?.nombre || 'Jugador') + '!';
});
