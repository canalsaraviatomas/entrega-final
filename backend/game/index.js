// --- Importación automática de módulos de lógica ---
import fs from "fs";
...
try {
  ...
  for (const file of fs.readdirSync(logicDir)) {
    if (file.endsWith(".js")) {
      ...
      logicModules[moduleName] = await import(path.join(logicDir, file));
    }
  }
} catch (e) {
  // Si se usa en frontend, ignora la importación dinámica
}

//Globales

// --- Lógica compartida: solo ejecutar en navegador ---
if (typeof window !== "undefined") {
  ...
  document
    .getElementById("copiarId")
    .addEventListener("click", () => copiarId());
  botonPreguntaAudio.addEventListener("click", () =>
    decir(document.getElementById("nombrePregunta").innerHTML)
  );
  ...
  const downloadBoard = () => {
    ...
    if (estadoPartida.turno != idJugador) {
      ...
      document.getElementById("turnoJugador").innerHTML =
        "Turno del jugador: " + estadoPartida.turno;
    }
  };
  ...
  function iniciar() {
    ...
    document
      .getElementById("enviarPregunta")
      .addEventListener("click", () => opcionElegida());
    fetch("/update", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        ...
        if (data.jugador1Id && !data.jugador2Id) {
          ...
          document.getElementById("entrada").style.background =
            `linear-gradient(90deg,${estadoPartida.color1Id} 50%,${estadoPartida.color2Id} 50%)`;
          guardarEstado();
        }
      });
  }
  ...
  const sendPing = () => {
    fetch("/ping", {
      ...
      body: JSON.stringify({
        playerId: idJugador,
        gameId: estadoPartida.tableroId,
      }),
...
// --- Exportar todos los módulos de lógica para uso externo ---
export { logicModules };
